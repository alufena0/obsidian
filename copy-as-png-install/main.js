...
// node_modules/modern-screenshot/dist/index.cjs
var require_dist = __commonJS({
  "node_modules/modern-screenshot/dist/index.cjs"(exports2) {
    "use strict";
...
`;
    ...
    async function domToCanvas2(node, options) {
      const context = await orCreateContext(node, options);
    ...
    async function domToBlob(node, options) {
      ...
      const canvas = await domToCanvas2(context);
    ...
    exports2.destroyContext = destroyContext;
    exports2.domToBlob = domToBlob;
    exports2.domToCanvas = domToCanvas2;
    exports2.domToDataUrl = domToDataUrl;
    exports2.domToForeignObjectSvg = domToForeignObjectSvg;
    exports2.domToImage = domToImage;
    exports2.domToJpeg = domToJpeg;
    exports2.domToPixel = domToPixel;
    exports2.domToPng = domToPng;
    exports2.domToSvg = domToSvg;
    exports2.domToWebp = domToWebp;
    exports2.loadMedia = loadMedia;
    exports2.waitUntilLoad = waitUntilLoad;
  }
});

// src/main.js
var { Plugin, MarkdownView, MarkdownRenderer, MarkdownRenderChild, Notice } = require("obsidian");
var { domToCanvas } = require_dist();
var CAPTURE_WIDTH = 1080;
var KEEP_PINNED_HEIGHT_TAGS = /* @__PURE__ */ new Set([
  "IMG",
  "VIDEO",
  "CANVAS",
  "SVG",
...
`;
var CopyAsPngPlugin = class extends Plugin {
  async onload() {
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, _editor, view) => {
        menu.addItem((item) => {
          item.setTitle("Copy note as PNG").setIcon("image").onClick(() => this.copyActiveViewAsPng(view));
        });
      })
    );
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        ...
        menu.addItem((item) => {
          item.setTitle("Copy note as PNG").setIcon("image").onClick(async () => this.copyFileAsPng(file));
        });
      })
    );
  ...
  async copyFileAsPng(file) {
    ...
    try {
      ...
      new Notice(`Copy as PNG failed: ${err.message}`, 1e4);
    } finally {
      wrap.remove();
    }
  }
    ...
    outer: for (let yEnd = h; yEnd > 0; yEnd -= chunkRows) {
      ...
      for (let y = yEnd - 1; y >= yStart; y--) {
        ...
        for (let i = base; i < base + rowBytes; i += 4) {
          if (Math.abs(data[i] - ref[0]) > tolerance || Math.abs(data[i + 1] - ref[1]) > tolerance || Math.abs(data[i + 2] - ref[2]) > tolerance || Math.abs(data[i + 3] - ref[3]) > tolerance) {
            contentBottom = y + 1;
            break outer;
          }
        }
