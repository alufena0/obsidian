...
module.exports = class DevToolsRibbonPlugin extends Plugin {
    onload() {
        const ribbonEl = this.addRibbonIcon('menu', 'Open app menu', () => {
            // @ts-ignore
            const { remote } = require('electron');
            const r = remote || require('@electron/remote');
            const menu = r.Menu.getApplicationMenu();
            if (menu) {
                menu.popup({ window: r.getCurrentWindow() });
            }
        });

        const svgIcon = ribbonEl.querySelector('.svg-icon');
        if (svgIcon) {
            svgIcon.outerHTML = `<svg class="svg-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" width="18" height="18" fill="none" style="stroke: var(--icon-color, currentColor); stroke-width: 32px; stroke-linecap: round; stroke-linejoin: round;">
